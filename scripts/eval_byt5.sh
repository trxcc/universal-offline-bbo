...
ckpt_path="/root/autodl-tmp/universal-offline-bbo/logs/baseline_byt5/runs/2025-01-18_13-18-32_seed42/checkpoints/last.ckpt"
...
for task in $tasks; do 
    for seed in {42..49}; do
        ...
        ((job_number++))
        run_with_retry "src/train_omnipred.py \
            experiment=byt5 \
            ++seed=${seed} \
            ++train=false \
            ++test_suites=${task} \
