...
def run_simulation(
    ...
    rvel = b2Vec2(
        desired_vel[0] + np.random.normal(0, 1e-6),
        desired_vel[1] + np.random.normal(0, 1e-6),
    )

    desired_vel2 = np.array([xvel2, yvel2])
    rvel2 = b2Vec2(
        desired_vel2[0] + np.random.normal(0, 1e-6),
        desired_vel2[1] + np.random.normal(0, 1e-6),
    )
