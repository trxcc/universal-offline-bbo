...
def zero_mean_standardize(y: torch.Tensor, y_mean: torch.Tensor, y_std: torch.Tensor) -> torch.Tensor:
    return torch.where(y_std != 0, (y - y_mean) / y_std, torch.zeros_like(y))

def min_max_y(y: torch.Tensor) -> torch.Tensor:
    y_min = y.min()
    y_max = y.max()
    return torch.where(y_max != y_min, 
                      (y - y_min) / (y_max - y_min), 
                      torch.full_like(y, 0.5))
...
def normalize_ys_from_different_tasks(
    ...
    for task in unique_tasks:
        ...
        if torch.any(bad_idx):
            ranks = torch.tensor(stats.rankdata(task_values.numpy()), dtype=torch.float32)
            bad_rank_percentiles = ranks[bad_idx] / len(task_values)
            bad_z_scores = torch.tensor(stats.norm.ppf(bad_rank_percentiles.numpy()), dtype=torch.float32)
