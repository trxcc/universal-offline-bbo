...
from src.tasks.mcts_transfer_task.functions.real_world_utils.rover_utils import (
    AABoxes,
    AdditiveCosts,
    ConstCost,
    ConstObstacleCost,
...
def create_rover_problem():
    ...
    func = lambda x: f(x * (ub - lb) + lb) * (-1)

    return func


def main():
    f = create_rover_problem()
    dim = 60
    x = np.random.uniform(np.zeros(dim), np.ones(dim))
